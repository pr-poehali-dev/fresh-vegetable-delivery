          ...
          {cart.length === 0 ? (
            ...
            <div className="px-6 py-4 flex flex-col gap-4">
                ...
                {cart.map(item => (
                  <div key={item.id} className="flex items-center gap-4 bg-background rounded-xl p-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-sm text-muted-foreground">{item.price} ₽ / кг</div>
                    </div>
              ...
              <div className="bg-background rounded-xl px-4 py-3">
                ...
                <div className="flex justify-between items-center text-lg font-heading font-bold mt-2 pt-2 border-t border-border">
                  <span>Итого</span>
                  <span className="text-veggie-green">{finalTotal} ₽</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs text-muted-foreground font-medium uppercase tracking-wider">Оформление</span>
                <div className="flex-1 h-px bg-border" />
              </div>
              ...
              <div>
                <p className="text-sm font-semibold text-foreground mb-3 flex items-center gap-2">
                  <span className="w-6 h-6 rounded-full bg-veggie-green text-white text-xs flex items-center justify-center font-bold">1</span>
                  Доставка
                </p>
              ...
              <div className="flex items-center gap-3">
                <div className="flex-1 h-px bg-border" />
                <span className="text-xs text-muted-foreground font-medium uppercase tracking-wider">Контакты</span>
                <div className="flex-1 h-px bg-border" />
              </div>
              ...
              <div className="flex flex-col gap-3">
                ...
                {/* Оплата баллами */}
                {user && user.points > 0 && (() => {
                  const maxPoints = Math.min(user.points, totalPrice + delivery);
                  return (
                ...
                <button
                  onClick={onOrder}
                  disabled={orderStatus === 'loading' || !orderName.trim() || !orderPhone.trim() || !orderAddress.trim()}
                  className="w-full btn-accent py-3.5 rounded-xl font-heading text-base font-semibold disabled:opacity-50"
                >
                  {orderStatus === 'loading' ? 'Отправляем...' : `Оформить заказ — ${finalTotal} ₽`}
                </button>
              </div>
